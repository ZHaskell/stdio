...
decodePrim :: Int
           -> (PrimArray Word8 -> Int -> a)
           -> Parser a
{-# INLINE decodePrim #-}
decodePrim n f = ensureN n >> Parser (\ bs k ->
        let (V.PrimVector pa offset len) = bs
            r = f pa offset
        in r `seq` k (V.PrimVector pa (offset + n) len) r)
